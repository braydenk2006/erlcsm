...
## Quality gate (Release 0)

Verified locally on 2026-08-02:

- `pnpm format:check` — pass
- `pnpm lint` — pass
- `pnpm typecheck` — pass (31/31)
- `pnpm test` — pass (31/31)
- `pnpm --filter @commandry/api test:integration` — pass (cross-tenant isolation)
- `pnpm build` — pass (44/44)
- Worker health.check job — succeeded against PostgreSQL + Redis
- Web smoke: `/`, `/sign-in`, `/docs/status`, `/api/health`, `/api/ready` — HTTP 200

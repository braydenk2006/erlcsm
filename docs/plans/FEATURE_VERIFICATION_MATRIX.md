...
Generated from the live capability registry (`@commandry/entitlements`). Every configured capability appears exactly once. A capability is **Verified Complete** only when it has a working UI + backend + persistence + tenant isolation + permission + entitlement enforcement.

## Summary

- Total capabilities: **125**
- Verified Complete: **32**
...
| Members                         | `core.members`              | Start-Up    | Verified Complete     | /app/staff + /api/members + invitations + members.max + tests |
| Departments                     | `core.departments`          | Start-Up    | Verified Complete     | /app/departments + departments.max + tests                    |
...
| Notifications                   | `core.notifications`        | Start-Up    | Verified Complete     | Notification model + bell + fan-out                           |
| Mobile access                   | `core.mobile`               | Start-Up    | Partially Implemented | Responsive layout                                             |
| Installable PWA                 | `core.pwa`                  | Start-Up    | Partially Implemented | Manifest (not offline)                                        |

## Community Integrations

| Feature                | Capability Key             | Lowest Plan | Status                | Evidence / Notes                                                                                                                             |
| ---------------------- | -------------------------- | ----------- | --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| Discord integration    | `discord.integration`      | Start-Up    | Partially Implemented | OAuth when configured                                                                                                                        |
| Roblox account linking | `roblox.account_linking`   | Start-Up    | Verified Complete     | @commandry/roblox live+mock; challenge; /app/settings; uniqueness/expiry; tests                                                              |
| Activity tracking      | `activity.tracking`        | Start-Up    | Verified Complete     | Activity Engine derives from participation ledger; /api/activity + /api/analytics/operations; metrics/compliance/timeline/leaderboard; tests |
| Shift tracking         | `shifts.tracking`          | Start-Up    | Verified Complete     | Shift engine (start/break/resume/end/correct/auto-close) emits ledger events; /api/shifts; /app/activity; integration tests                  |
| Session management     | `sessions.management`      | Start-Up    | Verified Complete     | Session engine (draft→scheduled→open→active→completed) + generic attendance; credit flows to ledger; /app/sessions; tests                    |
| Announcements          | `announcements.management` | Start-Up    | Verified Complete     | /app/announcements + publish + fan-out                                                                                                       |
...
| Player locations & map         | `server.locations`                 | Start-Up    | Partially Implemented | Simulator only — live PRC API returns no coordinates  |
| Callsigns                      | `server.callsigns`                 | Start-Up    | Verified Complete     | Callsign column                                       |
| Wanted stars                   | `server.wanted_status`             | Start-Up    | Partially Implemented | Simulator only — live PRC API returns no wanted level |
...
| Player-history correlation     | `server.player_history`            | Growth      | Verified Complete     | Player-history correlation                            |
...
| MDT                         | `cad.mdt`                  | Start-Up    | Partially Implemented | Dispatch detail (no dedicated MDT) |
...
| Multi-agency dispatch       | `cad.multi_agency`         | Growth      | Partially Implemented | Agencies model, single per call    |
